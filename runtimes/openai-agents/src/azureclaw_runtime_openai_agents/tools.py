...
def _platform_mcp_url() -> str:
    return os.environ.get("AZURECLAW_PLATFORM_MCP_URL", DEFAULT_PLATFORM_MCP_URL)
...
def _make_tool(name: str):
    ...
    description = f"Invoke the AzureClaw platform Foundry tool {name!r} via MCP."
    ...
    _tool.__azureclaw_tool_name__ = name  # type: ignore[attr-defined]
    return _tool
...
def register_foundry_tools(agent: Any) -> None:
    ...
    for tool in agent.tools or []:
        nm = getattr(tool, "name", None) or getattr(tool, "__azureclaw_tool_name__", None)
        if nm:
