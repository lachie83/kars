...
ENV_AGENT_DID = "KARS_AGENT_DID"
ENV_AGENT_NAME = "KARS_AGENT_NAME"
...
def _env_name() -> str:
    return os.environ.get(ENV_AGENT_NAME, "kars-agent")
...
class MeshClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.relay_url = (relay_url or os.environ.get("KARS_AGT_RELAY_URL") or DEFAULT_RELAY_URL)
        if not self.relay_url.endswith("/"):
            self.relay_url = self.relay_url + "/"
        self.registry_url = (
            registry_url
            or os.environ.get("KARS_AGT_REGISTRY_URL")
