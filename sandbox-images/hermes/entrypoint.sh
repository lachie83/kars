...
if [ -d "$MCP_BASE" ] && [ -n "$(ls -A "$MCP_BASE" 2>/dev/null)" ]; then
  echo "[kars-hermes] Translating /etc/kars/mcp/* → $HERMES_CONFIG mcp_servers"
  {
    ...
    for meta in "$MCP_BASE"/*/meta.json; do
      ...
      if [ -n "$bearer_env" ]; then
        # Hermes config supports env var substitution; ${ENV_VAR} expands at
        # config-load time. We don't need to materialize the actual token
        # here (avoids it leaking into a writable file in the container).
        echo "    headers:"
        echo "      Authorization: \"Bearer \${$bearer_env}\""
      fi
      # Whitelist of allowed tools (if set in meta.json)
      allowed=$(jq -r '.allowedTools // [] | join(",")' "$meta" 2>/dev/null || true)
      if [ -n "$allowed" ] && [ "$allowed" != "" ]; then
        echo "    allowed_tools: [$allowed]"
      fi
    done
  } > "$HERMES_CONFIG.mcp-fragment"
  # Merge into config.yaml. If config.yaml exists, append; else create.
  if [ -f "$HERMES_CONFIG" ]; then
    # Remove any prior mcp_servers block then append the fresh one
    # (yq would be nicer but we want zero extra deps)
    awk '/^mcp_servers:/{skip=1} /^[a-z]/{if (skip && !/^mcp_servers:/) skip=0} !skip' \
        "$HERMES_CONFIG" > "$HERMES_CONFIG.tmp"
    cat "$HERMES_CONFIG.mcp-fragment" >> "$HERMES_CONFIG.tmp"
    mv "$HERMES_CONFIG.tmp" "$HERMES_CONFIG"
  else
    mv "$HERMES_CONFIG.mcp-fragment" "$HERMES_CONFIG"
  fi
  rm -f "$HERMES_CONFIG.mcp-fragment"
fi
