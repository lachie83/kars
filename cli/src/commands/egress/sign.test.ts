...
describe("pushArtifact", () => {
  it("invokes oras with the right argv and verifies the blob digest", async () => {
    const yaml = "apiVersion: kars.dev/v1alpha1\nkind: EgressAllowlist\n";
...
describe("buildPatchArgv / patchKarsSandbox", () => {
...
describe("buildEmitManifestYaml", () => {
  ...
  it("emits a complete, byte-stable KarsSandbox patch with header comment", () => {
    const yaml = buildEmitManifestYaml(base);
    expect(yaml).toBe(
      [
        `# kars egress allowlist — digest=${base.digest} signer=keyless:fulcio`,
  ...
  it("parses cleanly as YAML and shape-matches the CRD (allowlistRef + GitOps marker)", async () => {
    ...
    expect(obj.apiVersion).toBe("kars.azure.com/v1alpha1");
    expect(obj.kind).toBe("KarsSandbox");
    expect(obj.metadata.name).toBe("demo-agent");
    expect(obj.metadata.namespace).toBe("kars-demo");
    expect(obj.metadata.annotations["kars.io/applied-via-gitops"]).toBe("true");
