...
2. Send encrypted messages to any agent on the mesh (another AzureClaw sandbox, an external operator, a specialist agent).
...
- "pair with the cluster", "connect to AzureClaw", "set up federation"
...
**When:** Before any other tool. If the user gives you a string starting with `azcp_1_…`, immediately call `mesh_pair` with it. Pairing state is persisted to `~/.azureclaw/pairings.json` — you only pair once per cluster.
...
**When:** The user asks who is on the mesh, or you want to find an agent with a specific capability (e.g., `"security-audit"`, `"offload"`, `"assistant"`). Without a capability, `discover` aggregates across well-known seed capabilities (azureclaw-agent, task-execution, cluster-controller, offload, mesh-peer, etc.) and returns a deduped list.
...
- **Proactive offload:** Modern AzureClaw sandboxes *self-announce* the moment they're ready and begin work immediately. You'll see phase `acknowledged` (👋) within ~20 s of `ready`. If you don't, the cluster may be running an older sandbox image and will fall back to the ping/upload/dispatch flow (still works, just slower).
...
| `❌ Not paired with any AzureClaw cluster` | No pairing persisted | Call `mesh_pair` with a valid token |
