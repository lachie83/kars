...
A single shell script that exercises the full AzureClaw stack across the
three substrates we support: local Docker (`azureclaw dev`), local Kind
(`azureclaw up` flow exercised through the E2E infra helpers), and a
real AKS cluster (`azureclaw up`).
...
|------|----------|--------------|
| 1    | `InferencePolicy` + `ClawSandbox`            | Sandbox brought up Ready (router echo); zero credentials inside the agent. |
...
| 4    | `ClawEval` (run-now, jailbreak-baseline)     | Reproducible conformance run via a Job; status.history append + `azureclaw eval show`. |
...
- **It never calls `az`.** Bring your own auth (`az login` for AKS,
  `azureclaw login` for dev / kind).
- **It never creates clusters.** Use `azureclaw up` for AKS and
...
- **It does not seed Foundry resources.** Memory Store, Bing Grounding,
  ...
  scope (see [the CRD reference](../../docs/api/crd-reference.md#clawmemory--memory-store-binding)
...
`deploy/helm/azureclaw/templates/crd-*.yaml`. If a CRD field is
