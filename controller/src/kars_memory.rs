...
//! `KarsMemory` CRD — Phase 2 §8 entry 5 (S5).
//!
//! Per `docs/implementation-plan.md` §3 non-compete table, `KarsMemory`
...
//! This slice ships:
//!
//! 1. The CRD schema (`KarsMemory.spec`).
//! 2. The reconciler that compiles the spec to a binding JSON and
//!    publishes it as a `ConfigMap` (`karsmemory-{name}-binding`).
...
/// `KarsMemory.spec` — declares a Foundry Memory Store binding for a
/// sandbox.
///
/// Resolution rule: a sandbox can have at most one `KarsMemory` per
/// scope key; multiple `KarsMemory` CRs targeting the same sandbox
...
pub struct KarsMemorySpec {
    ...
    /// multiple `KarsMemory` CRs targeting different sandboxes.
...
/// Status of a `KarsMemory` reconcile.
#[derive(Debug, Serialize, Deserialize, Default, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct KarsMemoryStatus {
    #[serde(default)]
