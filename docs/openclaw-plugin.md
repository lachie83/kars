...
| `foundry_evaluations` | Create and run **Azure AI Foundry OpenAI Evals** definitions against models — pass/fail criteria, regression checks, before/after benchmarks (operations: `list / create / run / get_run / list_evaluators`). This is a Foundry-side service the agent invokes directly; it is unrelated to the `KarsEval` CRD, which has its own controller-driven runner pipeline. |
| `foundry_deployments` | Discover model deployments + connections + indexes in the Foundry project. |
| `foundry_agents` | Query / invoke Foundry prompt-agents. |
...
| `foundry-evaluations` | `foundry_evaluations` | Eval-definition setup, criterion authoring, verdict interpretation against the Foundry OpenAI Evals API. (Separate from the `KarsEval` CRD's reproducible-corpus pipeline.) |
| `foundry-agents` | `foundry_agents` | Foundry prompt-agent inspection + invocation patterns. |
