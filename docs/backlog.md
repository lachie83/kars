...
| **E2E Encryption** | Signal Protocol (X3DH + Double Ratchet) via AgentMesh relay. KNOCK trust handshake. No plaintext fallback. Proven with hex dumps in `docs/e2e-encryption-proof.md`. Vendored SDK pending AGT mesh; patch index in `docs/agt-vendored-patch-audit.md`. |
