...
The relay and registry are operated by AzureClaw (`agentmesh` namespace, two small services). They are not trusted with content. The cryptographic primitives are libsodium / Signal Protocol; the inference router links the [`agentmesh`](https://crates.io/crates/agentmesh) crate (the AGT Rust SDK) from crates.io, and the OpenClaw plugin layer installs `@microsoft/agent-governance-sdk` from npm at sandbox-image build time. There is no in-tree fork of either SDK.
