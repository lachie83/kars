...
- **Operator UX:** `azureclaw up / add / dev / connect / handoff / mesh / policy learn / migrate / convert / claw attest` plus the operator TUI.
...
- **Make the signed OCI allowlist authoritative.** Today the inline `allowedEndpoints` field on `ClawSandbox` is the source of truth and the signed artifact (when present) is a parallel check. The plan is to make the signed artifact the only source of truth, with the inline field becoming a read-only convenience. See [`egress-proxy.md`](egress-proxy.md).
