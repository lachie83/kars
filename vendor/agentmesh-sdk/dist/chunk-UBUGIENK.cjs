...
var FileConfigLoader = class extends events.EventEmitter {
  baseDir;
  keysDir;
  sessionsDir;
  policyPath;
  ...
  constructor(options = {}) {
    ...
    this.keysDir = path__namespace.join(this.baseDir, "keys");
    this.sessionsDir = path__namespace.join(this.baseDir, "sessions");
    this.policyPath = path__namespace.join(this.baseDir, "policy.json");
  ...
  resolveBaseDir() {
    ...
    return path__namespace.join(os__namespace.homedir(), ".agentmesh");
  }
  /**
  ...
  savePolicy(policy, policyPath) {
    ...
    try {
      ...
      fs__namespace.writeFileSync(filePath, content, { mode: 384 });
    } catch (error) {
  ...
  startPolicyWatcher() {
    ...
    try {
      if (!fs__namespace.existsSync(this.policyPath)) {
        this.savePolicy({});
      }
      this.policyWatcher = fs__namespace.watch(this.policyPath, (eventType) => {
        if (eventType === "change") {
          this.reloadPolicy();
        }
      });
  ...
  persistSession(session) {
    ...
    try {
      const filePath = path__namespace.join(this.sessionsDir, `${session.sessionId}.json`);
  ...
  deleteSession(sessionId) {
    ...
    try {
      const filePath = path__namespace.join(this.sessionsDir, `${sessionId}.json`);
      if (fs__namespace.existsSync(filePath)) {
        fs__namespace.unlinkSync(filePath);
      }
    } catch {
...
var RegistryClient = class {
  ...
  async register(identity, options = {}) {
    ...
    try {
      ...
      throw new chunkFNHOFD2H_cjs.NetworkError(
  ...
  async lookup(amid) {
    try {
      ...
      if (response.status === 200) {
        ...
        throw new chunkFNHOFD2H_cjs.NetworkError(`Lookup failed: ${response.status}`, "LOOKUP_ERROR", response.status);
      }
    } catch (error) {
      if (error instanceof chunkFNHOFD2H_cjs.NetworkError) throw error;
      throw new chunkFNHOFD2H_cjs.NetworkError(
        `Registry connection error: ${error instanceof Error ? error.message : "Unknown error"}`,
        "REGISTRY_ERROR"
      );
  ...
  async search(options) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        throw new chunkFNHOFD2H_cjs.NetworkError(`Search failed: ${response.status}`, "SEARCH_ERROR", response.status);
      }
    } catch (error) {
      if (error instanceof chunkFNHOFD2H_cjs.NetworkError) throw error;
      throw new chunkFNHOFD2H_cjs.NetworkError(
  ...
  async getPrekeys(amid) {
    try {
      ...
      if (response.status === 200) {
        ...
        throw new chunkFNHOFD2H_cjs.NetworkError(`Prekey fetch failed: ${response.status}`, "PREKEY_ERROR", response.status);
      }
    } catch (error) {
      if (error instanceof chunkFNHOFD2H_cjs.NetworkError) throw error;
      return null;
...
exports.createFileConfigLoader = createFileConfigLoader;
exports.getTierLevel = getTierLevel;
//# sourceMappingURL=chunk-UBUGIENK.cjs.map
//# sourceMappingURL=chunk-UBUGIENK.cjs.map